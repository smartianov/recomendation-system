...
# Извиняюсь за опоздание. Я постарался максимально подробно расписать каждый
# шаг, ибо не во всех местах, на мой взгляд, код совсем уж очевидный, в виду
# того, что я старался сделать его наиболее производительным средствами NumPy,
# ограничившись от использования стандартных структур данных в Python. Я
# понимаю, что это не тот язык, где следует об этом думать, но все же, мне
# показалось, что это стоит учесть, тем более раз уж уже опоздал.
...
class RecSys():
    ...
    def __grade(self, user, film, *, day_conds=None, place_conds=None):
        ...
        # что второй подход тоже имеет место быть.
